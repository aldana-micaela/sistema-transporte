...
public class Flete extends Transporte {
	...
	public Flete(String matricula, double cargaMax, double capacidad, double costoKm, int cantidadAcompaniantes,
		...
		super(matricula, cargaMax, capacidad, false, costoKm, 0);
		this.cantidadAcompaniantes = cantidadAcompaniantes;
		this.costoPorAcompaniante = costoPorAcompaniante;
		
		if (cantidadAcompaniantes <= 0) {
			throw new RuntimeException ("NO SE PUEDE CREAR EL FLETE CON LA MATRICULA: "+ matricula + " PORQUE SU CANTIDAD DE ACOMPA�ANTES ES 0 (CERO)");	
		}
		if (costoPorAcompaniante <= 0) {
			throw new RuntimeException ("NO SE PUEDE CREAR EL FLETE CON LA MATRICULA: "+ matricula + " PORQUE SU COSTO POR ACOMPA�ANTES ES 0 (CERO)");	
		}
